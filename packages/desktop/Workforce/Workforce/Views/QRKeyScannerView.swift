...
private let workforceKeyMarker = "# Added by Workforce"

struct WorkforceSSHKey: Identifiable {
...
enum SSHKeyInstaller {
    ...
    static func addToAuthorizedKeys(_ publicKey: String) throws {
        ...
        let commentLine = "\(workforceKeyMarker) \(dateStr)"
    ...
    /// Returns keys that were added by Workforce (preceded by the marker comment).
    static func workforceKeys() -> [WorkforceSSHKey] {
        let fm = FileManager.default
        ...
        var keys: [WorkforceSSHKey] = []

        for (i, line) in lines.enumerated() {
            guard line.hasPrefix(workforceKeyMarker) else { continue }
            let dateStr = String(line.dropFirst(workforceKeyMarker.count)).trimmingCharacters(in: .whitespaces)
    ...
    static func removeKey(_ key: WorkforceSSHKey) throws {
        let fm = FileManager.default
        ...
        for (i, line) in lines.enumerated() {
            ...
            if parts.count >= 2 && parts[0] == key.keyType && parts[1] == key.keyData {
                ...
                if i > 0 && lines[i - 1].hasPrefix(workforceKeyMarker) {
