...
enum BeadsService {
    ...
    private static let beadsMarkerPrefix = "<!-- agenthub:beads"
    private static let beadsMarkerVersion = 2
    private static var beadsMarker: String { "<!-- agenthub:beads:v\(beadsMarkerVersion) -->" }
    ...
    private static func beadsInstructionVersion(in content: String) -> Int? {
        // Match versioned marker: <!-- agenthub:beads:v2 -->
        if let range = content.range(of: #"<!-- agenthub:beads:v(\d+) -->"#, options: .regularExpression) {
            let match = content[range]
            if let numRange = match.range(of: #"\d+"#, options: .regularExpression) {
                return Int(match[numRange])
            }
        }
        // Match legacy unversioned marker: <!-- agenthub:beads -->
        if content.contains("<!-- agenthub:beads -->") {
            return 1
        }
        return nil
    }
    ...
    private static func replaceBeadsBlock(in content: String, with block: String) -> String {
        ...
        let pattern = #"<!-- agenthub:beads(?::v\d+)? -->[\s\S]*?<!-- agenthub:beads(?::v\d+)? -->"#
