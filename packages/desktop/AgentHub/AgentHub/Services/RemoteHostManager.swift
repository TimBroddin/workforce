...
final class RemoteHostManager {
    ...
    private func discoverRemotePort(host: RemoteHost, completion: @escaping (Result<RemoteDiscovery, Error>) -> Void) {
        ...
        process.arguments = host.sshBaseArgs + [
            "echo PORT=$(cat ~/.agenthub/daemon.port 2>/dev/null || echo NOPORT); echo TOKEN=$(cat ~/.agenthub/daemon.token 2>/dev/null)"
        ]
            ...
            DispatchQueue.main.async {
                ...
                guard let portValue = portStr, portValue != "NOPORT", !portValue.isEmpty else {
                    completion(.failure(NSError(domain: "SSH", code: 1,
                        userInfo: [NSLocalizedDescriptionKey: "AgentHub not running on remote (no port file)"])))
                    return
                }
    ...
    func spawnAgent(host: RemoteHost, cwd: String, agentType: String = "claude", completion: @escaping (Result<String, Error>) -> Void) {
        // Use the HTTP API on the remote agenthub daemon (via SSH tunnel).
        guard let conn = connections[host.id], conn.status == .connected, conn.localPort > 0 else {
            completion(.failure(NSError(domain: "AgentHub", code: -1,
    ...
    /// Returns the local tunnel port and API token for a remote host, used to
    /// connect to `ws://localhost:<port>/ws/terminal/<agentId>?token=<token>`.
    func terminalEndpoint(for hostId: UUID) -> (port: UInt16, token: String?)? {
        guard let conn = connections[hostId], conn.status == .connected, conn.localPort > 0 else { return nil }
        return (port: conn.localPort, token: conn.apiToken)
    }
