...
// Throttled broadcast: coalesces rapid agent-list broadcasts into a single one
let broadcastTimer: ReturnType<typeof setTimeout> | null = null;
function broadcastAgentsThrottled(): void {
...
function handleSpawn(req: SpawnRequest): { agentId: string } | { error: string } {
  ...
  if (req.flags?.some((f) => BLOCKED_FLAGS.includes(f as any))) {
    return { error: `Blocked flag: ${req.flags.find((f) => BLOCKED_FLAGS.includes(f as any))}` };
  }
  ...
  const agentId = ptyManager.spawn({
    ...
    onTitle: (id, title) => {
      ...
      if (agent && agent.paneTitle !== title) {
        agent.paneTitle = title;
        agent.lastActivityAt = new Date().toISOString();
        store.persist();
        broadcastAgentsThrottled();
      }
    },
    onExit: (id) => {
      const agent = store.getAgent(id);
